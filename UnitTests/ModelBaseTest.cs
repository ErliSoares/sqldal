...
namespace UnitTests
{
    ...
    public class ModelBaseTest
    {
        ...
        #region ExportConfigurationModel
        public class ExportConfigurationModel
        {
            public ExportConfigurationModel()
            {
                FileNameOptions = new Dictionary<String, Func<String>>
            {
                { "{Year}", GetYear },
                { "{Month}", GetMonth },
                { "{Day}", GetDay },
            ...
            {
                return startdate.Month.ToString().PadLeft(2, '0');
            }
            ...
            {
                return GetExportCount(this.ClientCode, this.scheduleType, this.startdate.Date).ToString();
            }

            private static int GetExportCount(String clientCode, String exportType, DateTime date)
            {
                return 0;
            }

            private static String GetFileNameFormat(String clientCode, String exportType)
            {
                if (clientCode == "AAMC" && exportType == "HUMRRO" || exportType == "ACT")
                    return "";
                else
                    return "";
            }

            public String GetAutomatedExportFileName()
            {
                String fileName = GetFileNameFormat(ClientCode, this.scheduleType);

                foreach (var kvp in FileNameOptions)
            ...
            internal enum Occurs { Daily = 1, Weekly };
            internal enum DailyFrequency { OccursOnceAt = 1, OccursEvery };
            internal enum EndDateType { EndDate = 1, NoEndDate };
            internal enum ExportType { PMI = 1, AAMC, HUMRRO, ACT };

            private Dictionary<String, Func<String>> FileNameOptions;

            public Boolean Recurs { get; set; }
            public String ClientCode { get; set; }

            [DALSQLParameterName(Name = "Name")]
            public String scheduleName { get; set; }
            [DALSQLParameterName(Name = "Description")]
            public String scheduleDescription { get; set; }
            [DALSQLParameterName(Name = "Type")]
            public String scheduleType { get; set; }

            [DALSQLParameterName(Name = "Occurs")]
            public int rb_occur { get; set; }
            [DALSQLParameterName(Name = "DailyOccursEvery")]
            public int d_every { get; set; }

            [DALIgnoreAttribute]
            public String w_mon { get; set; }
            [DALIgnoreAttribute]
            public String w_tus { get; set; }
            [DALIgnoreAttribute]
            public String w_wed { get; set; }
            [DALIgnoreAttribute]
            public String w_thur { get; set; }
            [DALIgnoreAttribute]
            ...
            private Byte _occursOnDays = 0;
            [DALSQLParameterName(Name = "WeeklyOccursOnDays")]
            public Byte OccursOnDays
            {
                get { return _occursOnDays; }
                set
                {
                    _occursOnDays = value;
                    PopulateDayStringsFromDaysInt();
                }
            }
            [DALSQLParameterName(Name = "WeeklyOccursEvery")]
            public int w_every { get; set; }

            [DALSQLParameterName(Name = "DFType")]
            public int rb_DF { get; set; }
            [DALSQLParameterName(Name = "DFOccursOnceAt")]
            public TimeSpan DF_time { get; set; }
            [DALSQLParameterName(Name = "DFOccursEveryFrequency")]
            public int n_every { get; set; }
            [DALSQLParameterName(Name = "DFOccursEveryType")]
            public String n_everyType { get; set; }
            [DALSQLParameterName(Name = "DFStartingAt")]
            public TimeSpan DF_starttime { get; set; }
            [DALSQLParameterName(Name = "DFEndingAt")]
            public TimeSpan DF_endtime { get; set; }

            [DALSQLParameterName(Name = "DurationEndType")]
            public int rb_endDate { get; set; }
            [DALSQLParameterName(Name = "DurationStartDate")]
            public DateTime startdate { get; set; }
            [DALSQLParameterName(Name = "DurationEndDate")]
            public DateTime enddate { get; set; }

            public DateTime LastRunTime { get; set; }
            public DateTime NextRunTime { get; set; }

            public void PopulateOccursDaysInt()
            {
                String[] days = new String[] { w_sun, w_mon, w_tus, w_wed, w_thur, w_fri, w_sat };

                OccursOnDays = 0;
                for (int i = 0; i < days.Length; i++)
                    if (days[i] != null) OccursOnDays |= bDays[i];
            }

            public void PopulateDayStringsFromDaysInt()
            {
                w_sun = (OccursOnDays & bDays[0]) == bDays[0] ? "on" : "";
                w_mon = (OccursOnDays & bDays[1]) == bDays[1] ? "on" : "";
                w_tus = (OccursOnDays & bDays[2]) == bDays[2] ? "on" : "";
                w_wed = (OccursOnDays & bDays[3]) == bDays[3] ? "on" : "";
                w_thur = (OccursOnDays & bDays[4]) == bDays[4] ? "on" : "";
                w_fri = (OccursOnDays & bDays[5]) == bDays[5] ? "on" : "";
                w_sat = (OccursOnDays & bDays[6]) == bDays[6] ? "on" : "";
            }

            public DateTime GetFirstRunTime()
            {
                return GetNextRunTime(DateTime.Now, startdate);
            }

            private Boolean MultipleDailyCanOccurAgain(DateTime start, DateTime now)
            {
                if (now.TimeOfDay > DF_endtime) return false;

                //set temp time to the beginning of range
                TimeSpan tNext = DF_starttime;

                // loop through each interval until you get to next one
                while (tNext < start.TimeOfDay)
                {
                    switch (n_everyType)
                    {
                        case "H":
                            tNext += new TimeSpan(n_every, 0, 0);
                            break;
                        case "M":
                            tNext += new TimeSpan(0, n_every, 0);
                            break;
                        default: //S
                            tNext += new TimeSpan(0, 0, n_every);
                            break;
                    }
                }

                return tNext < DF_endtime;
            }

            private TimeSpan GetTimeOfNextMultipleDailyOccurance(DateTime start, DateTime now)
            {
                //if it can't happen today (and it occurs daily, weekly doesn't care), set hours adjust to 24 hours to run again the next day, otherwise it will occur again today
                TimeSpan hoursAdjust = rb_occur != (int)Occurs.Daily || MultipleDailyCanOccurAgain(start, now) ? new TimeSpan() : new TimeSpan(24, 0, 0);

                //set temp time to the beginning of range
                TimeSpan tmpStart = DF_starttime;

                // loop through each interval until you get to next one
                while (tmpStart <= now.TimeOfDay)
                {
                    switch (n_everyType)
                    {
                        case "H":
                            tmpStart += new TimeSpan(n_every, 0, 0) + hoursAdjust;
                            break;
                        case "M":
                            tmpStart += new TimeSpan(0, n_every, 0) + hoursAdjust;
                            break;
                        default: //S
                            tmpStart += new TimeSpan(0, 0, n_every) + hoursAdjust;
                            break;
                    }
                }

                if (tmpStart > DF_endtime)
                    tmpStart = DF_starttime;

                return tmpStart;
            }

            private TimeSpan GetTimeOfNextOncePerDayRun(DateTime now)
            {
                //it can still happen today, + 5 minutes for small timing discrepancies
                if (now.TimeOfDay < DF_time.Subtract(new TimeSpan(0, 5, 0)))
                {
                    return DF_time;
                }
                else
                {
                    //otherwise happens at the specified time X days from now
                    return DF_time.Add(new TimeSpan(d_every, 0, 0, 0));
                }
            }


            private Byte[] bDays = new Byte[] { 0x1, 0x2, 0x4, 0x8, 0x10, 0x20, 0x40 };

            private String[] GetDaysStringArray()
            {
                return new String[] { w_sun, w_mon, w_tus, w_wed, w_thur, w_fri, w_sat };
            }

            private Boolean WeeklyCanHappenToday(DateTime start, DateTime now)
            {
                return (OccursOnDays & bDays[(int)start.DayOfWeek]) == bDays[(int)start.DayOfWeek]
                    && now.TimeOfDay < DF_time
                    && now >= startdate;
            }

            // for a weekly schedule, determines the next day of the week it will run
            internal int DetermineNextDayWeeklyCanRun(DateTime start, out Boolean advanceWeeks)
            {
                advanceWeeks = false;
                Byte day = 0;
                int i = (int)start.DayOfWeek;
                //if the current time is greater than the scsheduled time of day, start immediately looking at the next day
                if (i == (int)start.DayOfWeek && start.TimeOfDay > DF_time) i++;
                for (; i <= 6 && day == 0; i++)
                {
                    day = (Byte)(OccursOnDays & bDays[i]);
                }

                if (day == 0)
                {
                    advanceWeeks = true;
                    for (i = 0; i < (int)start.DayOfWeek && day == 0; i++)
                    {
                        day = (Byte)(OccursOnDays & bDays[i]);
                    }
                }
            ...
            private DateTime GetNextWeeklyRunDay(DateTime start, DateTime now)
            {
                //determine next day it can happen
                Boolean advanceWeeks;
                int nextDay = DetermineNextDayWeeklyCanRun(start, out advanceWeeks);

                //advance next day to that day
                DateTime next = now;
                //advance to beginning of the week w_every weeks from now
                if (advanceWeeks)
                    next = next.AddDays((w_every) * 7 - (int)now.DayOfWeek);

                do
                {
                    next = next.AddDays(1);
                } while ((int)next.DayOfWeek != nextDay);
            ...
            /// 
            /// </summary>
            /// <param name="now">Should be DateTime.Now</param>
            /// <param name="start">The context in which to run this function.  Normally this will be the last run time of the job or the DateTime at which a new job first runs.</param>
            /// <returns></returns>
            public DateTime GetNextRunTime(DateTime now, DateTime start)
            {
                if (now < startdate) now = startdate;
                if (start < startdate) start = startdate;
                DateTime next = new DateTime();
                Occurs o = (Occurs)rb_occur;
                DailyFrequency df = (DailyFrequency)rb_DF;
                EndDateType endType = (EndDateType)rb_endDate;

                switch (o)
                {
                    case Occurs.Daily:
                        if (df == DailyFrequency.OccursOnceAt)
                        {
                            next = now.Date.Add(GetTimeOfNextOncePerDayRun(start));
                        }
                        else if (df == DailyFrequency.OccursEvery)
                        {
                            if (MultipleDailyCanOccurAgain(now, start))
                                next = now.Date.Add(GetTimeOfNextMultipleDailyOccurance(now, start));
                            else
                                next = now.Date.Add(GetTimeOfNextOncePerDayRun(start));
                        }
                        break;
                    case Occurs.Weekly:
                        if (df == DailyFrequency.OccursOnceAt)
                        {
                            //if weekly occurs today && can still happen today
                            if (WeeklyCanHappenToday(now, start))
                            {
                                next = start.Date.Add(DF_time);
                            }
                            else
                            {
                                next = GetNextWeeklyRunDay(now, start).Date.Add(DF_time);
                            }
                        }
                        else if (df == DailyFrequency.OccursEvery)
                        {
                            TimeSpan nextTimeOfDay = GetTimeOfNextMultipleDailyOccurance(now, start);

                            if (MultipleDailyCanOccurAgain(now, start))
                                next = now.Date.Add(nextTimeOfDay);
                            else
                                next = GetNextWeeklyRunDay(now, start).Date.Add(nextTimeOfDay);
                        }
                        break;
                }

                if (next > enddate && endType == EndDateType.EndDate)
                {
                    //expired
                    next = new DateTime();
                }
        ...
        [TestMethod()]
        public void ExportConfigurationModel_Test()
        {
            var target = new ExportConfigurationModel
            {
                ClientCode = "AAMC",
                d_every = 1,
                DF_endtime = new TimeSpan(),
                DF_starttime = new TimeSpan(),
                DF_time = new TimeSpan(),
                ...
                rb_occur = 1,
                Recurs = false,
                scheduleDescription = "First test schedule",
                scheduleName = "Test",
                scheduleType = "Sections",
                startdate = new DateTime(2011, 10, 22),
                w_every = 0,
                w_fri = null,
                w_mon = null,
                w_sat = null,
            ...
            var db = new SqlDBAccess();
            db.ValidateForDAL(target);
            db.PrepareParameters(target, true);
        }
        #endregion
        ...
        public class SetFlag : GetFlag
        {
            public String ModifiedUser { get; set; }
        }

        [TestMethod()]
        public void InheritedModelOutputPropertiesBug()
        {
            new SqlDBAccess().PrepareParameters(new SetFlag
            {
                ClientCode = "AAMC",
                ResultExamID = "1",
                VoidFlag = "",
